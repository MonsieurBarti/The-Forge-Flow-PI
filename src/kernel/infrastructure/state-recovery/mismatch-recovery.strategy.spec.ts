...
describe("MismatchRecoveryStrategy", () => {
  it("handles mismatch type", () => {
    const strategy = new MismatchRecoveryStrategy(makeRestoreUseCase(ok({} as never)));
  ...
  it("returns action=restored when restore succeeds", async () => {
    ...
    const strategy = new MismatchRecoveryStrategy(makeRestoreUseCase(ok(restoreReport)));
  ...
  it("returns action=skipped when restore fails with LOCK_CONTENTION", async () => {
    ...
    const strategy = new MismatchRecoveryStrategy(makeRestoreUseCase(err(lockError)));
  ...
  it("returns action=skipped when restore fails with BRANCH_NOT_FOUND", async () => {
    ...
    const strategy = new MismatchRecoveryStrategy(makeRestoreUseCase(err(notFoundError)));
  ...
  it("returns err(SyncError RESTORE_FAILED) for other errors", async () => {
    ...
    const strategy = new MismatchRecoveryStrategy(makeRestoreUseCase(err(otherError)));
