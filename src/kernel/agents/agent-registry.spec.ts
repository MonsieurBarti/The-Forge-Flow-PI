...
import { AGENT_REGISTRY, findAgentsByCapability, getAgentCard } from "./agent-registry";

describe("AGENT_REGISTRY", () => {
  it("has an entry for every AgentType enum value", () => {
    const allTypes = AgentTypeSchema.options;
    for (const agentType of allTypes) {
      expect(AGENT_REGISTRY.has(agentType), `Missing registry entry for "${agentType}"`).toBe(true);
    }
  });
  ...
  it("has no extra entries beyond AgentType enum", () => {
    ...
    for (const key of AGENT_REGISTRY.keys()) {
      expect(allTypes.has(key), `Unexpected registry entry "${key}"`).toBe(true);
    }
  });
});
